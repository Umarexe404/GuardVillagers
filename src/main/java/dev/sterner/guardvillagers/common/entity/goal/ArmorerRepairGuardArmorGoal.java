                    ...
                    if (isArmorerOrWeaponSmith && guard.getTarget() == null) {
                        if (mob.getVillagerData().profession().matchesKey(VillagerProfession.ARMORER)) {
                            for (int i = 0; i < guard.guardInventory.size() - 2; ++i) {
                                ...
                                if (itemstack.isDamaged() && itemstack.contains(DataComponentTypes.EQUIPPABLE) && itemstack.getDamage() >= itemstack.getMaxDamage() / 2) {
                                    this.villager = mob;
                                    return true;
                                }
                            }
    ...
    public void tick() {
        ...
        if (guard.distanceTo(villager) >= 2.0D) {
            ...
            if (profession.matchesKey(VillagerProfession.ARMORER)) {
                for (int i = 0; i < guard.guardInventory.size() - 2; ++i) {
                    ItemStack itemstack = guard.guardInventory.getStack(i);
                    if (itemstack.isDamaged() && itemstack.contains(DataComponentTypes.EQUIPPABLE) && itemstack.getDamage() >= itemstack.getMaxDamage() / 2 + guard.getRandom().nextInt(5)) {
